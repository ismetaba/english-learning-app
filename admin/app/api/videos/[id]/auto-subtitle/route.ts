...
function runAsync(cmd: string, args: string[], opts: { timeout: number; env?: NodeJS.ProcessEnv }): Promise<{ status: number; stderr: string }> {
...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (wavAge > 7) {
      ...
      const dlResult = await runAsync(ytDlp, [
        '--js-runtimes', 'node', '--remote-components', 'ejs:github',
        '-x', '--audio-format', 'wav',
        '--postprocessor-args', 'ffmpeg:-ar 16000 -ac 1',
        '-o', wavPath,
        `https://www.youtube.com/watch?v=${video.youtube_video_id}`,
      ], { timeout: 120000, env: ENV_WITH_PATH });

      if (dlResult.status !== 0) {
        return NextResponse.json({ error: `Download failed: ${dlResult.stderr.substring(0, 300)}` }, { status: 500 });
      }
    ...
    if (jsonMtime < wavMtime || !fs.existsSync(jsonPath)) {
      const wxResult = await runAsync(VENV_PYTHON, [
        WHISPERX_SCRIPT, wavPath, jsonPath,
        '--model', 'base.en', '--language', 'en',
      ], { timeout: 300000, env: ENV_WITH_PATH });

      if (wxResult.status !== 0) {
        return NextResponse.json({ error: `WhisperX failed: ${wxResult.stderr.substring(0, 300)}` }, { status: 500 });
      }
    }
